...
*/
	...
	while (tmp != NULL && tmp->next != NULL && !((c == 'a') ? ps_is_a_sorted(data) : ps_is_b_sorted(data)) && (ps_gotta_push(tmp, pivot, c) || tmp->next->next == NULL))
	{
		if (tmp->nb == pivot)
		{
			(c == 'a') ? ps_print_and_apply(data, "pb") : ps_print_and_apply(data, "pa");
			(c == 'a') ? ps_print_and_apply(data, "rb") : ps_print_and_apply(data, "ra");
			wit = 1;
		}
...
//		ps_display_stacks(data->stacks);	//TODO
	...
	if (wit == 1)
		(c == 'a') ? ps_print_and_apply(data, "rrb") : ps_print_and_apply(data, "rra");
}
