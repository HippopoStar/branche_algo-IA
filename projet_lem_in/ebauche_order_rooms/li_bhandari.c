...
/*   Updated: 2019/06/19 21:21:54 by lcabanes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		li_reverse_path(t_data *data)
{
	...
	while (j > 0 && pos > 0)
	{
		...
		*((*(data->map + i))->pipes + (*(data->map + j))->id) = (signed char)0;
		*((*(data->map + j))->pipes + (*(data->map + i))->id) = (signed char)
			(*((*(data->map + j))->pipes + (*(data->map + i))->id) == 1 ? -1 : 0);
		pos--;
