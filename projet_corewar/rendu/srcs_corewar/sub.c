...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/01 20:51:06 by cmiran            #+#    #+#             */
/*   Updated: 2019/07/20 20:48:19 by cmiran           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	verbose(int token, int id, int *av)
{
	if (g_env.option & OPT_C)
		...
		ft_printf("P %4i", id);
	ft_printf(" | sub r%i r%i r%i\n", (av[0] + 1), (av[1] + 1), (av[2] + 1));
}

void		sub(t_champ *champ, int *av, unsigned char ocp, unsigned short pc)
{
	(void)ocp;
	(void)pc;
	if (av[2] < 0)
