...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/01 21:30:10 by cmiran            #+#    #+#             */
/*   Updated: 2019/07/20 20:48:25 by cmiran           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		unsigned short pc)
{
	if (g_env.option & OPT_C)
		ft_printf("P %s%4i"RESET" | xor ", g_color[champ->token], champ->id);
	else
		ft_printf("P %4i | xor ", champ->id);
...
void		xor_(t_champ *champ, int *av, unsigned char ocp, unsigned short pc)
{
	if (av[2] < 0)
		return ;
