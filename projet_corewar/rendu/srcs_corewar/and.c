...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/01 21:29:26 by cmiran            #+#    #+#             */
/*   Updated: 2019/07/20 21:54:01 by cmiran           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	verbose(t_champ *champ, int *av, char ocp, unsigned short pc)
{
	if (g_env.option & OPT_C)
		ft_printf("P %s%4i"RESET" | and ", g_color[champ->token], champ->id);
	else
		ft_printf("P %4i | and ", champ->id);
...
void		and_(t_champ *champ, int *av, unsigned char ocp, unsigned short pc)
{
	if (av[2] < 0)
		return ;
